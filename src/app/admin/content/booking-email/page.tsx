...
import { getBookingConfirmationEmailContent, updateBookingConfirmationEmailContent, BookingConfirmationEmailContent } from '@/actions/content';
...
export default function BookingEmailAdminPage() {
  const [content, setContent] = useState<BookingConfirmationEmailContent | null>(null);
  ...
  useEffect(() => {
    async function loadContent() {
      setIsLoading(true);
      const loadedContent = await getBookingConfirmationEmailContent();
  ...
  const handleSave = async () => {
    if (!content) return;
    const result = await updateBookingConfirmationEmailContent(content);
    if (result.success) {
  ...
  return (
    <div>
      <PageTitle>Booking Confirmation Email</PageTitle>
      <Card>
        <CardHeader>
